...
//! by the webview's canvas/WebGPU). This crate turns a `varve-core` Scene into
...
use varve_core::{BlendMode, EngineColor, FillIR, PathPoint, SceneNode, Shape};
...
pub struct RenderItem {
    ...
    #[serde(default)]
    pub strokes: Vec<varve_core::Stroke>,
    #[serde(default)]
    pub effects: Vec<varve_core::Effect>,
...
/// `@varve/engine` `Primitive` type is the stable webview contract (ADR-0001).
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
#[serde(tag = "kind")]
...
pub fn build_render_ir_flat(
    walked: &[(varve_core::NodeId, &SceneNode, Option<varve_core::NodeId>)],
) -> Vec<RenderItem> {
    walked
        .iter()
...
mod tests {
    use super::*;
    use varve_core::{Affine, BlendMode, Circle, EngineColor, Point, Rect};
    ...
    fn build_render_ir_flat_nested_frames() {
        ...
        let scene = vec![
            SceneNode {
                ...
                children: vec![varve_core::NodeId(2), varve_core::NodeId(3)],
                component_id: None,
                slots: None,
                opacity: 1.0,
                blend_mode: BlendMode::Normal,
        ...
        assert_eq!(walked[0].0, varve_core::NodeId(1));
        assert_eq!(walked[0].2, None);

        assert_eq!(walked[1].0, varve_core::NodeId(2));
        assert_eq!(walked[1].2, Some(varve_core::NodeId(1)));

        assert_eq!(walked[2].0, varve_core::NodeId(3));
        assert_eq!(walked[2].2, Some(varve_core::NodeId(1)));

        assert_eq!(walked[3].0, varve_core::NodeId(4));
