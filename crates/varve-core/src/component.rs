//! Component definitions with typed slots (mirror of `@varve/scene/component.ts`).
//!
...
///
/// Mirrors `@varve/scene` `slotsSatisfied()`: returns `true` when all
...
///
/// Mirrors `@varve/scene` `IdGen.next()`.
