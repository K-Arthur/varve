...
- **JS:** 6890+ tests across 589+ files (2026-07-18, `pnpm test`, full workspace, 0 failures). Includes 285+ new Motion Mode tests. TypeScript typecheck: 15/16 packages clean (@strata/editor has 96 files with ~599 pre-existing errors from canvas/render/hitTest, Session 50+ intelligence modules, and unrelated Inspector sections). None of it blocks `pnpm dev`/`pnpm build` since Vite/esbuild doesn't type-check.
- **Typecheck regression prevention** (`scripts/audit-typecheck-regression.mjs`): compares failing-file set against `.typecheck-baseline.json`; exits non-zero if new files appear or total count grows. Integrated into `just gate` via `typecheck-regression` recipe. Update baseline: `node scripts/audit-typecheck-regression.mjs --update`.
...
## Hard rules
- No emoji anywhere (§4.4). SVG icons via Lucide `<Icon>` (outline) or Phosphor `<SolidIcon>` (filled) only.
...
| `@strata/ui` | **Built** | Tokens: color ramps, 3 themes, WCAG-AA audit, `tokens.css` generated from TS. Icons: typed Lucide `<Icon name label>` for outline and Phosphor `<SolidIcon name label>` for filled icons with a11y contract. `TOOL_ICONS` + `CHROME_ICONS` (Lucide outline) and `SOLID_TOOL_ICONS` + `SOLID_CHROME_ICONS` (Phosphor filled) maps. Components: APG `Button` (5 variants), `IconButton`, `Toolbar` (roving tabindex), `NumberInput` (drag-to-scrub, arrow inc/dec), `components.css` (token-styled). 20 tests. |
