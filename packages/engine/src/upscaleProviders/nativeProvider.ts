 ...
 * per-tile progress, honor cancellation, and publish the provider that
 * produced that job's result.
 */
export const nativeUpscaleProvider: UpscaleProvider = {
  ...
  async upscale(imageData, options, signal) {
    const result = await upscaleWithNativeMetadata(imageData, options, signal);
    return result.imageData;
  },

  async upscaleWithMetadata(imageData, options, signal) {
    return upscaleWithNativeMetadata(imageData, options, signal);
  },
};

async function upscaleWithNativeMetadata(
  imageData: ImageData,
  ...
  try {
    ...
    return {
      imageData: await decodeImageBytesToImageData(responseBytes(resultBytes)),
      executionProvider,
    };
  } finally {
    unlisten?.();
