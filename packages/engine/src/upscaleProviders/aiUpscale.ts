...
interface RealEsrganSession {
  session: InferenceSession;
  executionProvider: 'webgpu' | 'webgl' | 'wasm';
}

async function createSession(modelPath: string): Promise<RealEsrganSession> {
  ...
  // Use the same capability probe and ordering as the other browser ONNX
  // consumers. A provider entry is only a candidate: creating the real
  // Real-ESRGAN session below is the model-specific compatibility gate.
  let providers: string[] = ['wasm'];
  try {
    const { getBestOnnxProviders } = await import('../backgroundRemoval/environmentCapabilities');
    providers = await getBestOnnxProviders();
  } catch {
    // Capability probing is advisory; the bounded WASM session remains the
    // portable fallback when a browser refuses the probe.
  }
  if (!providers.includes('wasm')) providers.push('wasm');

  let lastError: unknown;
  for (const provider of providers) {
    if (provider !== 'webgpu' && provider !== 'webgl' && provider !== 'wasm') continue;
    try {
  ...
  throw lastError instanceof Error
    ? lastError
    : new Error('No browser ONNX execution provider could load the Real-ESRGAN model');
}
...
/**
 * Run the browser model and retain the provider selected for this job. If an
 * accelerated session is created but cannot complete the representative model
 * tile, release it before retrying once on WASM; never overlap the heavy
 * provider runs or return a partially written image.
 */
export async function upscaleWithRealEsrganWithMetadata(
  source: ImageData,
  modelPath: string,
  isCancelled: () => boolean,
): Promise<RealEsrganResult> {
  const ort = await import('onnxruntime-web');
  const selected = await createSession(modelPath);
  let selectedReleased = false;
  try {
    try {
      return {
        imageData: await runSession(ort, source, selected.session, isCancelled),
        executionProvider: selected.executionProvider,
      };
    } catch (error) {
      if (selected.executionProvider === 'wasm' || isCancelled()) throw error;
      // A model can pass accelerated session creation and still fail on an
      // operator, shape, or resource limit at first execution. Treat that as
      // a provider failure and make one clean WASM attempt.
      await selected.session.release().catch(() => {});
      selectedReleased = true;
      const fallback = await createWasmSession(ort, modelPath);
      try {
        return {
          imageData: await runSession(ort, source, fallback, isCancelled),
          executionProvider: 'wasm',
        };
      } finally {
        await fallback.release().catch(() => {});
      }
    }
  } finally {
    // The GPU failure branch releases its session before creating the
    // fallback, so do not release it twice. Successful runs and non-GPU
    // failures still release here.
    if (!selectedReleased) {
      await selected.session.release().catch(() => {});
    }
  }
...
/** Compatibility wrapper for callers that only need pixels. */
export async function upscaleWithRealEsrgan(
  source: ImageData,
  modelPath: string,
  isCancelled: () => boolean,
): Promise<ImageData> {
  const result = await upscaleWithRealEsrganWithMetadata(source, modelPath, isCancelled);
