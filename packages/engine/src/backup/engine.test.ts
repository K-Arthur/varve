...
describe('BackupEngine', () => {
  ...
  it('creates and lists snapshots', async () => {
    ...
    expect(backups[0]!.type).toBe('snapshot');
    expect(backups[1]!.type).toBe('snapshot');
  });

  it('retrieves backup manifest and document', async () => {
    const store = createMemoryBackupStore();
    const engine = new BackupEngine(store);
    ...
    expect(manifest!.documentChecksum).toBeTruthy();
    expect(manifest!.assetsIncluded).toBe(false);

    const document = await engine.getBackupDocument(result.backupId!);
    expect(document).toBe(doc);
  });

  it('deletes a backup and updates index', async () => {
    const store = createMemoryBackupStore();
    const engine = new BackupEngine(store);
    ...
    let backups = await engine.listBackups('proj-4');
    expect(backups.length).toBe(2);

    await engine.deleteBackup('proj-4', r1.backupId!);
    backups = await engine.listBackups('proj-4');
    expect(backups.length).toBe(1);
  ...
  it('filters backups by type', async () => {
    ...
    expect(snapBackups[0]!.type).toBe('snapshot');
  });

  it('applies retention and removes old backups', async () => {
    const store = createMemoryBackupStore();
    const engine = new BackupEngine(store);
    ...
    const backups = await engine.listBackups('proj-6');
  ...
  it('detects corrupted backup', async () => {
    ...
    const doc = makeDocumentJson('corrupt-test');
    const result = await engine.createBackup('proj-8', 'automatic', doc, 'Corrupt Test', 1);
