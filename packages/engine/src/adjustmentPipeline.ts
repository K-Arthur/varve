 ...
 */
export function effectPixelExpansion(
  filter: FilterIR,
  pixelsPerUnit = 1,
): [number, number, number, number] {
  const scale = Number.isFinite(pixelsPerUnit) ? Math.max(0.01, pixelsPerUnit) : 1;
  switch (filter.kind) {
    case 'blur':
      return [
    ...
    case 'motionBlur': {
      const pad = Math.ceil((Math.abs(filter.distance) * scale) / 2) + 1;
      return [pad, pad, pad, pad];
    }
    case 'mosaic': {
      const pad = Math.ceil(Math.abs(filter.blockSize) * scale);
      return [pad, pad, pad, pad];
    }
    case 'surfaceSmooth': {
      const pad = Math.ceil(Math.abs(filter.radius) * scale);
      return [pad, pad, pad, pad];
    }
    case 'edgeInk': {
      const pad = Math.ceil(Math.abs(filter.radius) * scale) + 1;
      return [pad, pad, pad, pad];
    }
    case 'sharpen': {
 ...
 */
export function totalEffectExpansion(
  filters: FilterIR[],
  pixelsPerUnit = 1,
): [number, number, number, number] {
  let l = 0,
  ...
  for (const f of filters) {
    const [el, et, er, eb] = effectPixelExpansion(f, pixelsPerUnit);
