...
describe('LineTool', () => {
  ...
  it('shows line draft during drag', () => {
    const ctx = makeCtx();
  ...
  it('creates a line shape on drag end', () => {
    const ctx = makeCtx();
    tool.onPointerDown?.(makePointerEvent(100, 100), ctx);
    tool.onPointerMove?.(makePointerEvent(300, 200), ctx);
  ...
  it('creates a line shape at the correct start position', () => {
    const ctx = makeCtx();
    tool.onPointerDown?.(makePointerEvent(100, 100), ctx);
    tool.onPointerMove?.(makePointerEvent(300, 200), ctx);
    tool.onPointerUp?.(makePointerEvent(300, 200), ctx);
  ...
  it('creates default-sized line on click-without-drag (below threshold)', () => {
    const ctx = makeCtx();
    tool.onPointerDown?.(makePointerEvent(100, 100), ctx);
  ...
  it('rejects pointer down when already dragging', () => {
    const ctx = makeCtx();
    tool.onPointerDown?.(makePointerEvent(100, 100), ctx);
    tool.onPointerMove?.(makePointerEvent(200, 200), ctx);

    const result2 = tool.onPointerDown?.(makePointerEvent(300, 300, { pointerId: 2 }), ctx);
    expect(result2.consumed).toBe(false);
  });
});
