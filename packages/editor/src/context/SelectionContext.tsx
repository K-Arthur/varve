...
export function SelectionProvider({ children, state, setState }: SelectionProviderProps) {
  ...
  const isSelected = useCallback(
    (id: NodeId) => state.selection.includes(id),
    [state.selection],
  );

  const selectedNodes = useCallback(
    () => state.selection.map((id) => state.document.nodes[id]).filter((n): n is SceneNode => Boolean(n)),
    ...
    () => {
      setState((s) => {
        ...
        for (const [id, node] of Object.entries(s.document.nodes)) {
          if (node && node.kind === kind && !(node as SceneNode).locked && (node as SceneNode).visible !== false) {
            candidates.push(id);
          }
        }
  ...
  const selectAllWithSameLayerColor = useCallback(
    () => {
      setState((s) => {
        ...
        const candidates: NodeId[] = [];
        for (const [id, node] of Object.entries(s.document.nodes)) {
          if (node && (node as SceneNode).layerColor === color && !(node as SceneNode).locked && (node as SceneNode).visible !== false) {
            candidates.push(id);
          }
        }
        return { ...s, selection: candidates };
  ...
  const value = useMemo<SelectionContextValue>(
    ...
    [
      state.selection,
      setSelection, toggleSelection, isSelected, selectedNodes,
      selectAllWithSameType, selectAllWithSameFill,
      selectAllWithSameLayerColor, selectAllOfType,
    ],
  );
