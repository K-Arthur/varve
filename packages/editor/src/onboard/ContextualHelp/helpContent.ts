...
const LOCAL_HELP_CONTENT: Record<string, HelpArticle> = {
  ...
  'tool:eyedropper': {
    ...
    summary: 'Sample colors from the screen or canvas.',
    body: "Click on any pixel to sample its color. The sampled color is applied to the selected object's fill, so select a layer first. In Chromium-based browsers the eyedropper can sample anywhere on screen; elsewhere it samples pixels inside the canvas.",
    keywords: ['eyedropper', 'color', 'pick', 'sample', 'pipette'],
    category: 'Tools',
