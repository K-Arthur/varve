...
async function findVerifiedAttachHead(
  ...
  for (const branch of candidates) {
    ...
    if (!head) continue;
    try {
      if ((await replayAndVerify(store, documentId, head.revisionId, rasterTileStore)).verified) {
        return { branch, head };
      }
    } catch {
...
export class EditorHistorySession {
  ...
  async attach(document: Document): Promise<AttachResult> {
    ...
    const verified = await findVerifiedAttachHead(
      this.store,
      this.documentId,
      branches,
  ...
  private async commitCapture(
    ...
    const label = options.label === 'Edit' ? labelForDiff(diff) : options.label;
...
function labelForDiff(diff: ReturnType<typeof diffDocuments>): string {
  ...
  if (counts.text > 0 && counts.added + counts.removed + counts.modified === 0) return 'Edit text';
