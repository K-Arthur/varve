...
describe('detectVariants', () => {
  ...
  it('returns empty candidates for document with no component instances', () => {
    ...
    doc = addNode(
      doc,
      makeShapeNode('n1', { kind: 'rect', x: 0, y: 0, w: 100, h: 50 }, { name: 'Rect' }),
    );
    const result = detectVariants(doc, '');
    expect(result.candidates).toEqual([]);
  });
  ...
  it('detects variant candidates from component instances with different fills', () => {
    ...
    const inst1 = makeFrameNode('i1', {
      w: 100,
      h: 50,
      name: 'Primary',
      children: [],
      componentId: 'btn',
      fill: { space: 'rgb', r: 0, g: 100, b: 200, a: 255 },
    });
    doc = addNode(doc, inst1);
    const inst2 = makeFrameNode('i2', {
      w: 100,
      h: 50,
      name: 'Secondary',
      children: [],
      componentId: 'btn',
      fill: { space: 'rgb', r: 100, g: 200, b: 0, a: 255 },
    });
