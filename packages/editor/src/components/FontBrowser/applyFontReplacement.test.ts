...
describe('applyFontReplacement', () => {
  ...
  it('ignores a malformed face override and uses the linked style identity', () => {
    let doc = createDocument('font-replacement-malformed-reference');
    const rootId = doc.pages?.[0]?.contentRoot;
    if (!rootId) throw new Error('fixture page root missing');
    const originalReference = { artifactHash: 'a'.repeat(64), postScriptName: 'Inter-Regular' };
    ...
    delete styled.fontFamily;
    delete styled.fontReference;
    styled.styleOverrides = { fontReference: 'corrupt persisted value' };
    doc = addChild(doc, rootId, styled);
    doc = {
      ...doc,
      styles: {
        'style-1': {
    ...
    expect(updated.nodes.styled).toMatchObject({
      kind: 'text',
      styleOverrides: {
        fontFamily: 'Noto Sans',
