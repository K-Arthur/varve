...
// Before the registry-backed system, DisclosureSection used sessionStorage
// with keys like "strata:inspector:disclosure:<title-slug>". This migration
// reads any existing legacy values and merges them into the registry state.
// After migration, legacy keys are deleted from sessionStorage.

const LEGACY_STORAGE_PREFIX = 'strata:inspector:disclosure:';
const MIGRATION_MARKER_KEY = 'strata:disclosure-migration-v1';

/**
 * Title-to-sectionId mapping for legacy disclosure slugs.
 * Generated by `title.toLowerCase().replace(/\s+/g, '-')`.
 */
const LEGACY_SLUG_MAP: Record<string, SectionId> = {
...
/** Migrate legacy sessionStorage disclosure state into the registry. */
export function migrateLegacyDisclosureState(
  current: SectionVisibilityState,
): SectionVisibilityState {
  if (typeof window === 'undefined') return current;
  try {
    if (window.sessionStorage.getItem(MIGRATION_MARKER_KEY) === '1') return current;
    let migrated = false;
    ...
    for (let i = 0; i < window.sessionStorage.length; i++) {
      ...
      const sectionId = LEGACY_SLUG_MAP[slug];
      if (!sectionId) continue;
      const existing = result[sectionId];
      const isCollapsed = raw === '0';
      if (existing) {
        if (existing.collapsed !== isCollapsed) {
          result[sectionId] = { ...existing, collapsed: isCollapsed };
        }
      } else {
        result[sectionId] = { collapsed: isCollapsed, hidden: false };
      }
      migrated = true;
    }
    if (migrated) {
      window.sessionStorage.setItem(MIGRATION_MARKER_KEY, '1');
    }
    return result;
  } catch {
    return current;
