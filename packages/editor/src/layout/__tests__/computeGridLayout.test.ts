...
function makeDoc(overrides: Partial<Document> = {}): Document {
  return {
    ...
    rootChildren: [],
    version: '1.2',
    pages: [{ id: 'page1', name: 'Page 1', contentRoot: 'root', backgrounds: [] }],
    activePageId: 'page1',
    ...overrides,
  };
}
...
describe('computeGridLayout', () => {
  ...
  it('debug explicit placement inside function with detailed check', () => {
    ...
    // Check internal state of the function
    const parent = docWithParent.nodes[parentId] as Record<string, unknown>;
    expect(parent).toBeDefined();
    
    // Check childA's gridPlacement via doc.nodes lookup
    const childA = docWithParent.nodes['a'];
    expect(childA).toBeDefined();
    const childAGP = (childA as Record<string, unknown>).gridPlacement as Record<string, unknown>;
    expect(childAGP).toBeDefined();
    expect(childAGP.columnStart).toBe(2);
    
    // Verify children list in parent
    const children = parent.children as string[];
    expect(children).toEqual(['a', 'b']);
    
    // Both results must exist
    expect(result).toHaveLength(2);
  ...
  it('debug explicit placement inside function', () => {
    const doc = addNode(addNode(makeDoc(), 'a', {
      gridPlacement: { columnStart: 2, columnEnd: 3, rowStart: 1, rowEnd: 2 },
    }), 'b');
    const parentId = 'parent';
    ...
    expect(docWithParent.nodes['a']).toBeDefined();
    expect(docWithParent.nodes['b']).toBeDefined();
    expect(docWithParent.nodes['parent']).toBeDefined();
    
    // Verify gridPlacement on child
    const childA = docWithParent.nodes['a'] as Record<string, unknown>;
    expect(childA.gridPlacement).toBeDefined();
    ...
    expect(result).toHaveLength(2);
    expect(result[0]?.id).toBe('a');
    // 'a' should be at column 2 (1-indexed), which is col=1 (0-indexed) → x=200
    expect(result[0]?.x).toBe(200);
  });
  ...
  it('handles uneven row and column counts', () => {
    ...
    const result = computeGridLayout(docWithParent, parentId, 300, 200, ls, ['a', 'b', 'c']);
  ...
  it('handles empty children', () => {
    ...
    const ls = makeLayoutStyle();
    const result = computeGridLayout(doc, parentId, 400, 200, ls, []);
