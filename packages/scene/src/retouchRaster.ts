 ...
 * edge. Source and destination are separate ownerships even when they belong
 * to the same raster layer; all reads come from `sourceTiles`.
 */
...
function blendRetouchPixel(
  ...
  return {
    r: clampByte(
      (source.r * sourceAlpha + destination.r * destinationAlpha * (1 - sourceAlpha)) /
        outputAlpha,
    ),
    g: clampByte(
      (source.g * sourceAlpha + destination.g * destinationAlpha * (1 - sourceAlpha)) /
        outputAlpha,
    ),
    b: clampByte(
      (source.b * sourceAlpha + destination.b * destinationAlpha * (1 - sourceAlpha)) /
        outputAlpha,
    ),
