...
describe('reflowLayoutChildren', () => {
  ...
  it('a hug grid frame sizes itself from explicit px and auto tracks', () => {
    const frame = makeFrame({
      w: 999,
      h: 999,
      layoutSizingWidth: 'hug',
    ...
    const resolved = reflowed.nodes.f1;
    if (resolved?.kind === 'frame') {
      // width: 60 + 80 + gap(8) + padding(4+4) = 156;
      // height: 30 + padding(4+4) = 38.
