...
export function createMemoryPlatform(options: MemoryPlatformOptions = {}): Platform {
  ...
  const platform: Platform = {
    ...
    async saveBinaryFile(name: string, data: Uint8Array, _mimeType: string, _extension: string) {
      state.files.set(name, {
        entry: {
          ...
          pinned: false,
          trashedAt: null,
          contentHash: contentHash(Array.from(data).map((b) => String.fromCharCode(b)).join('')),
        },
        json: '',
      });
      return `memory://${name}`;
