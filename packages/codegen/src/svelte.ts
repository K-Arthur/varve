...
import type { Document as SceneDocument, SceneNode, VariableStore } from '@varve/scene';
...
function buildTemplate(
  ...
  if (
    ...
    (node as import('@varve/scene').FrameNode).layoutStyle
  ) {
    const ls = (node as import('@varve/scene').FrameNode).layoutStyle!;
...
export function exportNodeToSvelte(
  ...
  if (opts?.useRunes !== false) {
    ...
    parts.push(`  // Generated by @varve/codegen svelte emitter`);
