...
function shapeVerticesToPoints(s: { kind: string } & Record<string, unknown>, precision: number): string {
  if (s.kind === 'polygon') {
    const cx = Number(s.cx), cy = Number(s.cy), radius = Number(s.radius);
    const sides = Number(s.sides), rotation = Number(s.rotation);
    const pts: string[] = [];
    for (let i = 0; i < sides; i++) {
      const a = (2 * Math.PI * i) / sides - Math.PI / 2 + rotation;
      pts.push(`${fmtNum(cx + radius * Math.cos(a), precision)},${fmtNum(cy + radius * Math.sin(a), precision)}`);
    }
    return pts.join(' ');
  }
  if (s.kind === 'star') {
    const cx = Number(s.cx), cy = Number(s.cy), ir = Number(s.innerRadius), or = Number(s.outerRadius);
    const points = Number(s.points), rotation = Number(s.rotation);
    const pts: string[] = [];
    for (let i = 0; i < points * 2; i++) {
      const a = (Math.PI * i) / points - Math.PI / 2 + rotation;
      const r = i % 2 === 0 ? or : ir;
      pts.push(`${fmtNum(cx + r * Math.cos(a), precision)},${fmtNum(cy + r * Math.sin(a), precision)}`);
    }
    return pts.join(' ');
  }
  return '';
 ...
 */
export function computeDocumentBounds(doc: Document): { x: number; y: number; w: number; h: number } {
  ...
  for (const id of doc.rootChildren) {
    ...
    if (node.kind === 'shape') {
      const s = node.shape;
      switch (s.kind) {
        case 'rect': bx = s.x; by = s.y; bw = s.w; bh = s.h; break;
        case 'ellipse': bx = s.cx - s.rx; by = s.cy - s.ry; bw = s.rx * 2; bh = s.ry * 2; break;
        case 'circle': bx = s.cx - s.r; by = s.cy - s.r; bw = s.r * 2; bh = s.r * 2; break;
        case 'line': bx = Math.min(s.from[0], s.to[0]); by = Math.min(s.from[1], s.to[1]); bw = Math.abs(s.to[0] - s.from[0]); bh = Math.abs(s.to[1] - s.from[1]); break;
        case 'polygon': bx = s.cx - s.radius; by = s.cy - s.radius; bw = s.radius * 2; bh = s.radius * 2; break;
        case 'star': bx = s.cx - s.outerRadius; by = s.cy - s.outerRadius; bw = s.outerRadius * 2; bh = s.outerRadius * 2; break;
      }
    } else if (node.kind === 'text') {
      bw = node.text.length * node.fontSize * 0.6;
      bh = node.fontSize * 1.2;
    } else if (node.kind === 'frame') {
      bw = 200; bh = 200; // approximate
    }
  ...
  if (!hasNodes) return { x: 0, y: 0, w: 1920, h: 1080 };
...
function nodeToSvg(
  node: SceneNode, doc: Document, depth: number, options: SvgExportOptions,
): string {
...
export function exportDocumentToSvg(doc: Document): string {
  return exportDocumentToSvgAdvanced(doc, {});
}

/** Export a Document to SVG with advanced options. */
export function exportDocumentToSvgAdvanced(doc: Document, options: SvgExportOptions): string {
