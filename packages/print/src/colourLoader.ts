/**
 * @strata/print — WASM colour module loader.
 *
 * Loads the `strata_colour` wasm-pack build from the public `/wasm/` directory.
 * Follows the same pattern as `wasmLoader.ts` in @strata/engine.
 *
 * Two WASM candidates are tried in order:
 *   1. `/wasm/strata_colour_bg.wasm`  (baseline)
 *
 * The loader fetches the wasm-bindgen glue JS as text, creates a blob URL
 * (workaround for Vite dev server restrictions on dynamic import of /public
 * assets), imports it, and initialises the WASM module.
 */
...
async function tryLoadCandidate(wasmUrl: string): Promise<ColourWasmModule | null> {
  try {
    // HEAD check: does the .wasm file exist?
...
/**
 * Load the colour WASM module. Cached after first successful load.
 * Returns `null` if the module cannot be loaded (no WASM support,
 * missing assets, fetch failure).
 */
...
/**
 * Create a colour engine from a loaded WASM module.
 */
...
/**
 * Colour engine interface for browser-side colour operations.
 */
