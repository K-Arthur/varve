...
test.describe('warp: direct manipulation latency', () => {
  ...
  test('cage and artwork track the latest pointer sample', async ({ page }) => {
    ...
    // Pixel-grid snapping (on by default) may rest the handle within one
    // snap cell of the raw pointer; the strict equality checks below compare
    // two equivalent drags, where snapping cancels out.
    expect(Math.abs(settledReal.x - endX)).toBeLessThan(8);
    expect(Math.abs(settledReal.y - endY)).toBeLessThan(8);
    ...
    // Snap tolerance: resting within one snap cell of the burst endpoint.
    expect(Math.abs(last.x - synthEnd.x)).toBeLessThan(8);
    expect(Math.abs(last.y - synthEnd.y)).toBeLessThan(8);
