...
test.describe('Image crop — interactive tool', () => {
  ...
  /** Helper: create a rect on the canvas and return its bounding box. */
  async function createRectAndSelect(page: import('@playwright/test').Page) {
    await page.keyboard.press('r');
    const box = await dragOnCanvas(page, 150, 150, 350, 300);
    await expect(page.getByRole('treeitem')).toHaveCount(1, { timeout: 10000 });
    // Switch to select tool and click the shape to select it
    await page.keyboard.press('v');
    await page.waitForTimeout(200);
    // Click center of the rect to select it
  ...
  test('C key enters crop mode for selected shape', async ({ page }) => {
    ...
    await createRectAndSelect(page);
    // Enter crop mode
