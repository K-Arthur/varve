...
const scratch = join(ROOT, '.capture-tmp', runId);
...
try {
  ...
  const exit = await new Promise((resolve) => runner.on('close', resolve));
  assert.equal(exit, 0, `native WDIO flow failed with exit ${exit}`);
  ...
  await toWebm(raw, webm, { start: 0, fps: 30 });
  await toMp4(raw, mp4, { start: 0, fps: 30 });
  await posterFrom(webm, poster);
  const delivered = await probe(webm);
  const verification = {
    findings: [],
    clips: {},
    environment: { native: true, display, artifact: binary },
  };
  ...
  if (!process.argv.includes('--keep-source')) rmSync(scratch, { recursive: true, force: true });
