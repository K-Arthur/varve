 ...
 *
 * Varve.s version lives in nine places that must agree, or the installer lies
 * about what it is (audit RB-3: all nine read `0.0.0`, and `publish.yml` never
 ...
 *
 * `kind: 'appstream-releases'` manages `<release version="X.Y.Z">` entries in
 * AppStream metainfo XML.  On set/bump, a new entry is inserted above existing
 * ones; on set to the same version that is already topmost, the date is
 * refreshed.  Prerelease versions are written as-is — AppStream permits any
 * string — so the maintainer has full control.
 *
 * `kind: 'pkgver-line'` targets the `pkgver=X` line in an AUR PKGBUILD.
 * Only the version literal on that line is rewritten; source URLs that use
 * `${pkgver}` are automatically resolved by makepkg.
 *
 * `kind: 'srcinfo-version'` targets the `pkgver = X` line and version-bearing
 * `source =` lines in a `.SRCINFO` (generated by `makepkg --printsrcinfo`).
 * The sha256sums field is never touched — it must be refreshed from the
 * published SHA256SUMS.txt after artifacts are built.
 */
const TARGETS = [
...
function readTarget(target) {
  ...
  if (target.kind === 'appstream-releases') {
    const match = text.match(/<release\s+version="([^"]+)"/);
    return { text, value: match?.[1] ?? undefined };
  }
  if (target.kind === 'pkgver-line') {
    const match = text.match(/^pkgver=(.+)$/m);
    return { text, value: match?.[1] };
  }
  if (target.kind === 'srcinfo-version') {
    const match = text.match(/^\tpkgver\s*=\s*(.+)$/m);
    return { text, value: match?.[1] };
  }
...
function writeTarget(target, version) {
  ...
  if (target.kind === 'appstream-releases') {
    ...
    const oldTopmost = readTarget(target).value;
  ...
  if (target.kind === 'srcinfo-version') {
    ...
    updated = updated.replace(
      /(\tsource\s*=\s*Varve-)[^:]+(-linux)/g,
      (_m, prefix, suffix) => `${prefix}${version}${suffix}`,
    );
    updated = updated.replace(
      /(releases\/download\/v)[^/]+(\/Varve-)[^"']+(")/g,
      (_m, prefix, mid, suf) => `${prefix}${version}${mid}${version}-linux-x86_64.AppImage${suf}`,
    );
    if (updated === text && readTarget(target).value !== version) {
      throw new Error(`Could not locate pkgver or source in ${target.path}`);
    }
    writeFileSync(abs, updated);
    return;
  }
...
function applyVersion(version, actionLabel) {
  ...
  process.stdout.write(
    'After publishing the release, refresh AUR sha256sums from the published\n' +
      'SHA256SUMS.txt:\n' +
      `  curl -sL https://github.com/K-Arthur/varve/releases/download/v${version}/SHA256SUMS.txt | grep x86_64.AppImage\n` +
      'then update packaging/aur/varve-desktop-bin/PKGBUILD sha256sums and run\n' +
      'makepkg --printsrcinfo > packaging/aur/varve-desktop-bin/.SRCINFO\n',
  );
}
