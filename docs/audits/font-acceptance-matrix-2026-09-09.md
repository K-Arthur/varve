...
| 12 | A missing family, missing face, missing glyph, corrupt file, version mismatch, and restricted font each produce a distinct state and recovery action. | Open | Resolver and Missing Fonts UI retain distinct status labels and exact recovery-row keys; family-only legacy records and same-family exact artifacts no longer share recovery metadata. Complete capability/preflight and recovery actions for every status remain pending. |
