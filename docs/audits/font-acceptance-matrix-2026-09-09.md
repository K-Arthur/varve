...
| 22 | Image font identification uses the selected crop, handles cancel and low confidence, previews candidates, and applies to an explicit text target or creates editable text. | Partial | `FontDetectSection` now uses the existing canvas crop tool, follows crop/rotation/flip settings during bounded extraction, offers a full-image fallback, supplies the live registry/local comparison dependencies, and queues **Use for new text** with an optional exact reference. Candidate overlays, OCR-assisted region selection, and applying a result to an existing text target remain pending. |
