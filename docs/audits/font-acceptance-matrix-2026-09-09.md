...
| 7 | Real and unavailable weights/styles produce accurate controls; synthetic styling is blocked or clearly disclosed according to product policy. | Partial | Registry-backed weight options now drive the inspector, contextual/floating toolbars, and Logo panel. Static faces expose registered weights; variable faces expose their declared `wght` range; stale legacy values remain visible but disabled, and the floating Bold action is blocked when 700 is unavailable. Synthetic italic disclosure, style-specific native proof, and full missing-face recovery remain pending. |
