...
| M7 | **Not fixed (documented).** Enabled `reason` strings from the declarative menu model are still not rendered by the HTML menubar; the two renderers remain separate. Contributed menus still need depth/cycle validation at the command-graph layer. | Architectural: `buildMenus` in `Menubar.tsx` is a separate renderer from `menu/defs.ts` + `renderer.ts`. | Low–Medium (discoverability of unavailable commands) | Left as remaining work; not silently half-implemented. |
