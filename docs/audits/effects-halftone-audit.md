...
**Date:** 2026-07-06 | **Scope:** `@strata/engine`, `@strata/scene`, `@strata/editor`, `@strata/codegen`, `crates/strata-print`, `crates/strata-core`
...
| FM/stochastic + blue-noise screening | Print RIPs, creative-coding shaders | Strata already implements both AM and FM (Floyd-Steinberg) halftone screening |
...
| 7 | `scene/adjustments.ts` (and its re-exports, including `AdjustmentLayerNode`/`Adjustment`/`AdjustmentKind`) was never exported from `@strata/scene`'s `index.ts` barrel | `scene/index.ts` | `AdjustmentPanel.tsx` and `AdjustmentLayerRow.tsx` imported names from `'@strata/scene'` that did not exist on the public API — a live compile break | **Fixed**: added `export * from './adjustments'` |
| 8 | `HalftoneAdjustment` itself was never added to `@strata/engine`'s or `@strata/scene`'s re-export lists (both list every other `*Adjustment` type by name) | `engine/src/index.ts`, `scene/src/adjustments.ts` | Any code importing `HalftoneAdjustment` by name (including the new halftone editor UI) failed to typecheck | **Fixed** |
...
- **No per-frame effect-output caching.** Only the halftone threshold *matrix* is now memoized (§3.14). The full pixel computation (screening, curves, etc.) still reruns every render call. `packages/compositor/src/canvas2d/tileCache.ts` exists but `Canvas2DBackend.drawVectorItems` never calls `.has()` — draws are never skipped, by explicit design comment ("immediate-mode canvas is cleared each frame").
...
| P2 | Per-content-hash caching of full effect *output* (not just the halftone matrix) | Requires a cheap content-fingerprinting strategy and interacts with the immediate-mode canvas redraw model (`tileCache.ts`'s existing "always replay" design decision) |
...
| P3 | Per-ink customizable CMYK screen angles (Affinity-style single angle vs. Photoshop-style per-channel) | Current fixed standard angles are moiré-safe; making them user-editable needs UI + moiré-risk messaging |
