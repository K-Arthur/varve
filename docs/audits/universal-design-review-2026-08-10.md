...
- E2E: motion (14/14), print-mode-preflight (3/3), workspace-switch and cross-mode round-trip, toolbar-per-mode, and tooltip 7/8 verified green before the parallel session's engine refactor broke the web boot. As of the closing commit, the working tree does not start (`renderPipeline.ts` imports `workerSourceCapFor`, which their in-flight `collectImageBitmaps.ts` no longer exports), so no further E2E can run until their refactor lands — re-run the green set plus `workspace-mode`, `motion-mode`, `drawing-mode-focus`, and `ownership` then. `drawing-mode-focus` (pencil Stabilization) and `ownership.spec` also assert inspector internals mid-refactor by that same session.
