...
remains actionable so the user can return to regular. The change prevents an
apparently successful click from creating an unsupported synthetic style or selecting another file that
...
The follow-up identity audit adds four collision regressions: a variable axis
from a different same-family artifact or collection member cannot affect an
exact static face's weight or style controls. The font-policy suite now passes
21 assertions in total.
