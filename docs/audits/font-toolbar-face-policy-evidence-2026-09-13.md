...
The quick text toolbar now treats Italic as a real-face capability. It stays
disabled for a regular-only family, moves to a same-artifact italic sibling
when one exists, and recognizes a declared variable `ital` axis. A selected
italic face remains actionable so the user can return to regular. The change
prevents an apparently successful click from creating an unsupported synthetic
style or selecting another file that merely shares the family name.
