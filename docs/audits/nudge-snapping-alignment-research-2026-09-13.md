...
| Raw pointer intent after a snap correction | **Working in solver and indexed canvas path** | Move a target, then drag a second node from a raw pointer proposal into its edge. | High | `snapPosition` accepts `rawIntent`; the SelectTool applies one correction to the raw total drag rather than feeding the previous correction back. | Pointer gestures in browser/desktop. | `tools/__tests__/snapping.test.ts`; `tests/e2e/canvas/snap-after-move.spec.ts` (moved, created, and reopened targets). |
| Sticky-lock target replacement/deletion | **Working in solver; deletion invalidation live test pending** | Replace a same-coordinate target and sample again. | High | `SnapLock` records target/source/feature identity and scope filtering invalidates it. | Indexed canvas path. | `tools/__tests__/snapping.test.ts`; add live target deletion E2E when the delete/cancel fixture is isolated. |
...
| Alignment reference/key-object feedback | **Working in Inspector and page/frame UI paths** | Select a frame child, activate parent-frame then page references, align left, and inspect the active reference plus world result. | High | Feedback derives from post-command document and records reference identity. | Chromium Inspector path; menu/shortcut overlay path remains narrower. | `AlignDistributeBar.test.tsx`, arrangement tests, `tests/e2e/canvas/alignment-arrangement.spec.ts`. |
...
| Focused text/number field or toolbar | **Working for canvas/idle-tool routing; widget browser proof pending** | Canvas and idle Frame-tool Arrow paths passed; widget-focused routing remains owned by the focused control. | High | `shouldIgnoreShortcutTarget`, IME guard, and canvas ownership boundary. | Chromium canvas path; WebKitGTK/native focus should be checked separately. | Shortcut tests; keyboard-navigation and Settings E2E. |
| Guides describe applied result and stay camera-locked | **Working for live movement and alignment** | During a moved-target drag the dashed guide and final Inspector value identify the same target edge; alignment uses post-command lines. | Medium | `AlignmentGuideOverlay` consumes solver/result lines and `worldToScreen` camera mapping. | Chromium browser at 1280×800; rotated-camera browser and desktop renderer remain unverified. | Alignment visual E2E plus rotated-camera screenshot. |
| Save/reopen and export after movement | **Working for save/reopen → snap; export placement pending** | Move target, save with Ctrl+S, reload, reopen the recent document, then snap a second node to the persisted target; exact X equality passes. | High | Persistence and index rebuild agree with the Inspector and guide output after reopen. | Chromium browser IndexedDB; native file path remains unverified. | `tests/e2e/canvas/snap-after-move.spec.ts`; add export-specific placement assertion if export bounds are extended. |
