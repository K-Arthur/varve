...
|---|---|---|---|---|
| A | Effect masks on shadow/glow/backdrop effects remain unsupported | H (renderer, now declared) | `packages/engine/src/replay.ts`, `shadowSource.ts`, `packages/editor/src/render/groupEffectStages.ts` | Content-stage masks resolve in live structural replay and structured export for scene-node, vector, and document-owned raster sources. The Inspector keeps authoring disabled for appearance/backdrop masks until their mask-combination semantics are specified; authored masks on those types are now reported through the engine diagnostic channel (`effect-mask-unsupported`) in both live and export, so they are never mistaken for applied masks. Implementing the renderer half still needs the mask-combination contract. |
...
| E | Effect allocation refusal / failed pixel reads were silent in the engine and live renderer | M (fixed) | `replay.ts`, `effectPipeline.ts`, `shadowSource.ts`, `renderPipeline.ts` | **Fixed** (fe5f4ab48, group stages follow-up): `replayIr` reports typed effect diagnostics through `onEffectDiagnostic`; the shared group/frame stages report from their surface/mask/depth/backdrop catches; structured export maps them into `ExportSnapshot.diagnostics`/warnings; live replay announces them (bounded, deduplicated) from CanvasArea. Remaining blind spots: `shadowSource.ts` null-buffer fallbacks and `filterCompositor`'s unused `onDiagnostic`. |
...
| Tier 0 audits | `pnpm audit:tokens` / `audit:emoji` / `audit:docs` (direct script reruns after pnpm's shared-install guard) | tokens 153 pairs pass; emoji clean across 4,635 files; docs clean (834 docs, 458 links). An intermediate emoji attempt saw a concurrent missing config, but the final rerun was clean after that file returned. |
| E2E typecheck | `pnpm typecheck:e2e` | earlier run only reported the concurrent `packages/engine/src/depthMap.ts(846)` error; final rerun is still blocked by unrelated `packages/engine/src/areaSelectionMorphology.ts(565-566)` unused parameters and `backgroundRemoval/maskDecode.ts(35)` typed-array/Blob incompatibility; zero errors in the new spec |
...
| Effect diagnostics (engine → group stages → export → live) | `pnpm exec vitest run --maxWorkers=1 packages/editor/src/render/replayScene.test.ts packages/editor/src/render/groupEffectStages.test.ts packages/editor/src/export/compositor.test.ts packages/engine/src/replay.test.ts` | 12 + 3 + 84 + 64 passed; new `effect-mask-unsupported` reporting test and `effect-degraded` export test prove the engine→compositor channel; group/frame stage catches forward the same diagnostics |
| Hardened group-opacity E2E | `VARVE_E2E_PORT=1495 pnpm exec playwright test tests/e2e/canvas/group-opacity-alpha.spec.ts --project=chromium` (started from `pnpm exec playwright test --grep "colored backdrop" --retries=1`) | Test 1 passed (1.5 m run); test 2 passed and was marked flaky only because its first attempt's drag registered 2 of 3 layers under load — the retry passed. Spec now patches opacity by node id (`data-node-id` + `updateNode`) and waits for an actual pixel change instead of fixed delays; earlier per-case passes remain valid. |
| Engine typecheck | `pnpm --filter @varve/engine typecheck` | 7 pre-existing other-session errors (colorization, contentAwareFill, lut); zero in `replay.ts`/`index.ts` |
...
| Architecture health | `node scripts/audit-architecture.mjs --ci` | exited 0: 14 existing cycles, no layer violations, and no new violation in the fidelity helper/compositor paths. Existing instability and hub-budget warnings remain outside this slice. |
| Escalated full gate | `VARVE_FULL_GATE_REASON="layer fidelity cross-package milestone and escalated shared-checkout plan" pnpm verify:full` | Could not start: pnpm detected a concurrent modules-directory purge and aborted with `ERR_PNPM_ABORTED_REMOVE_MODULES_DIR_NO_TTY`; no shared dependency reinstall was forced. |
