...
| E | Effect allocation refusal / failed pixel reads were silent in the engine and live renderer | M (fixed) | `replay.ts`, `effectPipeline.ts`, `shadowSource.ts`, `renderPipeline.ts` | **Fixed** (fe5f4ab48): `replayIr` reports typed effect diagnostics through `onEffectDiagnostic`; structured export maps them into `ExportSnapshot.diagnostics`/warnings, live leaf replay announces them (bounded, deduplicated) from CanvasArea, and export rasterization failures use the same channel. Remaining blind spot: `shadowSource.ts` null-buffer fallbacks and `filterCompositor`'s unused `onDiagnostic`. |
...
| Effect diagnostics (engine → export → live) | `pnpm exec vitest run --maxWorkers=1 packages/editor/src/render/replayScene.test.ts packages/engine/src/replay.test.ts`; `pnpm exec vitest run --maxWorkers=1 packages/editor/src/export/compositor.test.ts` | 12 + 64 passed (new `effect-mask-unsupported` reporting test); 84 passed (new `effect-degraded` export test proving the engine→compositor channel) |
