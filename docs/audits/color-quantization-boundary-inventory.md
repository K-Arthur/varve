...
| Raster pixel buffer descriptor | Typed `rgba8`, `rgba16`, `rgba16f`, `rgba32f` storage plus encoding/alpha metadata | Format is selected by owner | No | Yes | **Implemented:** budgeted allocator, half-float helpers, and non-mutating `convertPixelBufferFormat()` quantization boundary; decode/cache integration remains separate. |
