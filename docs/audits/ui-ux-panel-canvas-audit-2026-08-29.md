...
- **PNL-11 (Panel Collapse Restoration Affordances & SVG Iconography)**: Floating direct-manipulation restore buttons (`.editor__panel-restore-btn--left`, `.editor__panel-restore-btn--right`) render on the canvas margin when side panels are collapsed, positioned at `top: calc(var(--topbar-height, 40px) + 32px + 28px)` to clear the document tab bar and ruler systems without collision. Replaced generic chevrons with bespoke 16x16 SVG sidebar-collapse and sidebar-expand icons.
- **PNL-12 (Splitter Drag Hit Target Ergonomics)**: Expanded `.panel-resize` handle hit target width from 4px to 14px (`inline-size: 14px; left: -7px; right: -7px;`) in `editor.css` to enable smooth mouse and trackpad dragging without visual divider line bloat.
- **PNL-13 (Top Left Sidebar Collapse Relocation & Conspicuous Iconography)**: Relocated the left panel collapse affordance from `LayersPanel/index.tsx` (where it was previously buried halfway down the rail next to `PanelDetachButton`) to the top of the left sidebar inside `MinimapPanel.tsx` (in the minimap header, collapsed minimap bar, and fallback header). Elevated `.editor__collapse-btn` to an instrument-grade, conspicuous control with micro-border, surface elevation, drop shadow, and bespoke 16x16 SVG icon with tinted sidebar region and directional collapse chevron, matching the top floating expand button for spatial symmetry and muscle memory.
