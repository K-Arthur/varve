...
- `VARVE_E2E_PORT=1453 VARVE_E2E_WORKERS=1 pnpm exec playwright test tests/e2e/effects/spatial-filters.spec.ts --project=chromium --reporter=list`: passed again after adding the real precision-field Undo/Redo assertion for Motion Blur (1 test, 26.4 seconds).
...
- `pnpm exec vitest run packages/engine/src/effectRegistry.test.ts --maxWorkers=1`: 8 passed after replacing array-shape colour guessing with explicit colour parameter keys; palette and gradient stop arrays remain structured data.
- The same registry test now covers operation-specific metadata for hue rotation and opacity (`degrees` and `percent` ranges), avoiding the generic `value` range where it would be misleading.
- The registry now marks Motion Blur, Mosaic, Surface Smooth and Edge Ink as bounds-expanding, matching their compositor support and the object-transform-aware capture padding.
- `CHANGELOG.md` now records the four additions, compositing fix, transaction/persistence behavior and metadata corrections under `Unreleased`.
...
- Rebuilt the static marketing site with `pnpm --filter @varve/website build:pages`, ran the focused website E2E color-feature checks (2 passed) and palette extraction/mobile check (1 passed), and inspected `website-effects-light-latest.png` plus `website-effects-mobile-dark-latest.png`. The new filter names fit the existing product scope copy without introducing mobile overflow.
