...
| Save and reopen a masked adjustment | The new browser workflow saved through File → Save, reloaded to Home, reopened the library document, and inspected serialized intent plus the live mask source picker | Adjustment-specific save/reopen coverage was missing even though the canonical scene codec already owned the mask and stack data | Keep mask, scope, stable adjustment entry id, and parameters in authoritative serialized scene state; keep previews derived; exercise the existing browser persistence adapter | Chromium `clipping-masks.spec.ts --grep "saved adjustment masks"`: 1 passed; before/after screenshots `reports/masking-review/10-adjustment-mask-{before,after}-reopen-canvas.png` inspected |
...
- LUT library portability and raster/PDF export still need a fresh full workflow
  capture on this checkout. Adjustment masks now have targeted Chromium
  render and browser save/reopen passes (pixel oracle, serialized intent, and
  live inspector screenshot), but this does not establish every export format
  or native desktop file-dialog behavior. The direct
