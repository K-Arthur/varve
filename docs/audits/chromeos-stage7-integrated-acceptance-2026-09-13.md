...
| Policy audits | `pnpm audit:docs`; `pnpm audit:emoji`; direct fallbacks `node scripts/audit-docs.mjs`, `node scripts/audit-emoji.mjs`, and `./packages/ui/node_modules/.bin/tsx packages/ui/scripts/audit-tokens.ts` | Direct fallbacks: docs 834 docs, 458 links, 174 ADRs indexed, clean; emoji 4,632 files scanned, clean; tokens all 153 contrast pairs pass across 3 themes. The pnpm wrappers were blocked by their no-TTY module-state guard | `IMPLEMENTED + AUTOMATED-TESTED` for the direct audit scripts; wrapper rerun `NOT RUN` |
