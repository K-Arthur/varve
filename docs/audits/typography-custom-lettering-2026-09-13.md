...
|---|---|---|---|---|---|---|
| Feature values and ranges | Boolean, numeric/indexed, and UTF-16-ranged values survive scene → backend conversion; whole-run browser values use a process-local `FontFace` descriptor when an inspectable local CSS source exists | Canvas2D cannot portably address ranged glyph IDs; native/WASM adapters can | shared, engine, scene, editor, Rust shaper | A value can otherwise be silently reduced to on/off | normalized shared map, structured native wire request, required-feature warning, exact-source browser alias for whole-run values | shared/engine/native contract tests plus Chromium `liga`-off pixel/hash check |
...
| [CSS Font Loading Module Level 3](https://www.w3.org/TR/css-font-loading/) | `FontFace` can be constructed with `featureSettings` and `variationSettings`, loaded explicitly, and added to a `FontFaceSet`; those descriptors affect the constructed face rather than font selection. | Use an ephemeral alias only for whole-run browser paint when the authored CSS source can be inspected; keep source-range settings on the exact shaping path and never serialize the generated family. |
...
5. `89aca9d0d` protected ligatures across replay, path, and deformation; the
   ...
   `163640101` connects desktop outline conversion to native shaping. The
   browser whole-run path was then repaired with exact `FontFace` descriptors
   in `b08b683ef`; readiness repaint and selection lifecycle fixes followed in
   `657d2a78c` and `2c03803d9`.
