...
| Text discovery | Grounding DINO Tiny (INT8) explicit local download, phrase-attributed boxes, automatic routing never downloads | image-backed reviewed source box → mapped canvas envelope + exact source-space segmenter hint | detection never commits a mask; the selected box must be explicitly confirmed against the source image; scores are model similarity, not proof of presence; no match is an honest empty state |
