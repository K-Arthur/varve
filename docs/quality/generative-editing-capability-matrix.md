...
| Model-frame aspect conversion | Shared contract-driven letterbox transform; source and mask use the same scale/offset and restore to the exact context geometry | SD 1.5 diagnostic profile: 512 × 512, 64-pixel multiple; non-square contexts are letterboxed, never stretched | SDXL 1024 × 1024 transform contract is implemented for future qualification only; no SDXL provider is enabled | `packages/engine/src/generativeEdit/diffusionFrame.test.ts`, `packages/engine/src/generativeEdit/diffusionFrame.ts`; architecture contract in `docs/architecture/generative-editing-system.md` |
