...
| Hugging Face candidate selection | Research ledger and `packages/engine/src/generativeEdit/modelProfiles.ts` distinguish direct masked inpainting/outpainting from reference editors, record complete local component/runtime/license requirements, and keep candidates out of routing until a real adapter and qualification evidence exist | Current prompt profile remains disabled; promptless local routes are the only supported low-memory path | SD 2 Inpainting is retained as a failed diagnostic profile; PowerPaint v2-1 and FLUX.1-Fill-dev are selected for future high-memory local qualification; FIBO-Edit is a third comparison candidate; Qwen/FLUX.2/Kontext remain reference-edit research | [Local model landscape 2026-09-15](../research/generative-inpainting-model-landscape-2026-09-15.md) |
