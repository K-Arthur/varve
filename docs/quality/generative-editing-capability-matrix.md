...
| Model-frame aspect and mask conversion | Shared contract-driven letterbox transform; source and mask use the same scale/offset and restore to the exact context geometry. Each profile declares masked-inpainting versus reference-edit input and white/black mask polarity; the masked path rejects reference-only providers and converts canonical 0-preserve/255-edit coverage explicitly. New persisted diffusion provenance records the allowlisted model profile, input kind, mask convention, pre-letterbox context, and integer content rectangle | SD 1.5 diagnostic profile: 512 × 512, 64-pixel multiple, white-edit/black-preserve; SD 2 research profile has a separate 512 × 512 contract and adapter requirement; neither is quality-certified | SDXL 1024 × 1024 transform contract is implemented for future qualification only; no SDXL provider or accelerated backend is enabled | `packages/engine/src/generativeEdit/diffusionFrame.test.ts`, `packages/engine/src/generativeEdit/diffusionFrame.ts`, `packages/scene/src/generativeEdit.test.ts`; architecture contract in `docs/architecture/generative-editing-system.md` |
