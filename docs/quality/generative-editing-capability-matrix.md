...
| Expand without a prompt | Fast/PatchMatch stays blocked for Expand (visible striping on real-photo review, unchanged). AI quality (browser worker, once the model is installed) reaches the same shared reconstruction pipeline as Fill/Remove as of 2026-09-16 — engine-gated and unit-tested; a real-photo browser qualification pass (Playwright, downloaded model) has not yet run. See [the real-model check](../audits/lama-reconstruction-real-model-check-2026-09-16.md) | Implemented; LaMa model-backed when the pinned local model is installed, deterministic reconstruction otherwise. Source pixels are copied through exactly; the full requested border including corners is generated; effective generated detail is bounded by the model frame and disclosed in review. A shared-decode nearest-neighbor→bilinear fix (2026-09-16, [evidence](../audits/lama-decode-interpolation-2026-09-16.md)) removed a real block/pixelation artifact at the generated border reproduced across ten real photographs, including the flagged architecture case; the underlying "architecture continuation limited/review-only" resolution boundary is unchanged and still needs a fresh visual re-review against the frozen corpus, not just this probe. Progressive/iterative margin growth (repeated smaller passes instead of tiling) was compared against the shipped staged-border tiling on large single-direction margins and performed clearly worse in two different implementations (solid black regions or corrupted noise) — not implemented; see [the comparison](../audits/lama-progressive-expand-comparison-2026-09-16.md). The tile-seam banding that comparison flagged was measured and partly addressed (2026-09-16, [evidence](../audits/lama-tile-seam-context-padding-2026-09-16.md)): widening AI-quality tile context padding from 32px to 96px gave a measured ~12% seam reduction with no downside and is now the default; a feather-blend across tile boundaries was also tried and made the worst-measured seam clearly worse (ghosting from blending two misaligned independent generations), so it was not implemented. Large many-tile margins can still show some residual seam | Linux x86_64 CPU only; Windows, macOS, ARM, and constrained package evidence pending | [Expand qualification 2026-09-13](../audits/generative-expand-qualification-2026-09-13.md), [browser real-model check 2026-09-16](../audits/lama-reconstruction-real-model-check-2026-09-16.md), [decode interpolation fix 2026-09-16](../audits/lama-decode-interpolation-2026-09-16.md), [progressive-vs-staged comparison 2026-09-16](../audits/lama-progressive-expand-comparison-2026-09-16.md), [tile-seam context padding 2026-09-16](../audits/lama-tile-seam-context-padding-2026-09-16.md), `crates/varve-bgremove/tests/lama_expand_qualification.rs`, `packages/engine/src/generativeEdit/expandPlan.test.ts`, `packages/engine/src/generativeEdit/expandFallback.test.ts`, `packages/engine/src/generativeEdit/generativeEdit.test.ts`, `packages/engine/src/inference/models/lama.test.ts`, `tests/e2e/caf/expand-real-photo.spec.ts` |
| Expand frame controls | Visible for capability explanation; Generate is disabled at Fast quality only. AI quality (once the model is installed) can generate here as of 2026-09-16, pending real-browser qualification evidence | Shared dialog path converts only containing frames to margins; no crop or source resampling | Desktop pointer/keyboard coverage and package qualification pending | `packages/editor/src/components/ContentAwareFill/expandControls.test.ts`, `tests/e2e/caf/expand-real-photo.spec.ts` |
...
| Object Selection → Generative Edit mask | Implemented; workflow-wired and automatically verified; full-resolution SAM2 allocation is safe-peak preflighted. Candidates are source-locator guarded, remain reviewable, and stale proposals are discarded when the image changes in place. Automatic foreground proposals now keep the overlay visible after candidate preview but require an explicit review confirmation keyed to the exact proposed mask/alpha bytes; switching candidates clears it. The decoder removes encoder letterbox padding before restoring source geometry. After decoding, candidates must contain include points, exclude background points, and overlap a supplied box; rejected candidates are removed from cycling and the separate prompt-match score is shown alongside predicted IoU. | Shared implementation; confirmed candidate only; native OS/cgroup snapshot used when available, package qualification pending | Shared implementation; confirmed candidate only; package qualification pending | `tests/e2e/caf/object-selection-mask-source.spec.ts`, `tests/e2e/canvas/object-selection-real-model.spec.ts`, `packages/editor/src/context/promptedSegmentationProvider.test.ts`, `packages/editor/src/context/useSam2Segmentation.test.tsx`, `packages/editor/src/components/Inspector/SelectionSourcesPanel.subject.test.tsx`, `packages/engine/src/inference/resourcePolicy.test.ts`; [selection/model audit](../audits/generative-editing-selection-model-audit-2026-09-14.md); SAM2 quality remains a separate real-model gate |
...
| Model-frame aspect and mask conversion | Shared contract-driven letterbox transform; source and mask use the same scale/offset and restore to the exact context geometry. Each profile declares masked-inpainting versus reference-edit input, white/black mask polarity, and mask tensor conditioning (`image-and-mask`, `masked-image-and-mask`, `masked-image-plus-mask`, or `none`); the masked path rejects reference-only providers and converts canonical 0-preserve/255-edit coverage explicitly. New persisted diffusion provenance records the allowlisted model profile, input kind, mask convention, mask input shape, pre-letterbox context, and integer content rectangle | SD 1.5 diagnostic profile: 512 × 512, 64-pixel multiple, white-edit/black-preserve, image-plus-mask; SD 2 research profile has a separate 512 × 512 contract and adapter requirement; neither is quality-certified | SDXL 1024 × 1024 transform contract is implemented for future qualification only; MI-GAN and Moebius low-level mask contracts are research-only and have no production adapters | `packages/engine/src/generativeEdit/diffusionFrame.test.ts`, `packages/engine/src/generativeEdit/diffusionFrame.ts`, `packages/scene/src/generativeEdit.test.ts`; architecture contract in `docs/architecture/generative-editing-system.md` |
| Hugging Face candidate selection | Research ledger and `packages/engine/src/generativeEdit/modelProfiles.ts` distinguish direct masked inpainting/outpainting from reference editors, record complete local component/runtime/license requirements, and keep candidates out of routing until a real adapter and qualification evidence exist | Current prompt profile remains disabled; promptless local routes are the only supported low-memory path | MI-GAN is screened as a tiny promptless Fill/Remove candidate but requires a vision.cpp sidecar; Moebius is screened as a fixed-512 promptless Fill/Remove candidate but requires a three-graph ONNX/DDIM adapter. Neither is qualified. SD 2 Inpainting remains a failed diagnostic profile; PowerPaint v2-1 was dropped from the registry on 2026-09-16 (Python/Diffusers/PyTorch runtime, poor match for the low-end/CPU-first target, and an environment check found no available machine safe to even start adapter work on); FLUX.1-Fill-dev remains a future high-memory four-mode candidate for a possible server/hosted-provider iteration; FIBO-Edit is a third comparison candidate; Qwen/FLUX.2/Kontext remain reference-edit research | [Local model landscape 2026-09-15](../research/generative-inpainting-model-landscape-2026-09-15.md); [lightweight-model screen](../audits/generative-editing-lightweight-model-screen-2026-09-16.md); [PowerPaint scope decision](../plans/powerpaint-adapter-qualification-scope-2026-09-16.md) |
...
`diffusion-rs-0.1.20-varve-image-cfg-profile-contract-v2`. The helper is supervised in a
...
- A corrected production-helper rerun on a real coastal photograph also
  ...
  provenance. That evidence used the prior helper identity; the current
  profile-bound v2 helper requires a fresh qualification run and remains
  unqualified. See the [native boundary probe](../audits/generative-editing-native-probe-2026-09-14.md).
