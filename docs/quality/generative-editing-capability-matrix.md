...
| Browser source-buffer preflight | Implemented; automatically verified | Explicit browser memory hints include resident source and temporary preparation buffers before AI background-removal allocation; unknown hints retain provider gating and Quick fallback | Shared browser path; physical Chromebook/ARM package qualification pending | `packages/engine/src/backgroundRemoval/__tests__/sourceMemoryPreflight.test.ts`, `packages/engine/src/inference/resourcePolicy.test.ts` |
