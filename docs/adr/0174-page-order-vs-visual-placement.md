# ADR-0174: Page order versus visual placement
...
D2 — Visual placement (`Page.placement` / `Spread.placement`, ADR-0173) never
...
persisted when custom spreads exist (ADR-0177); reorder re-projects spread
...
- Diff must classify reorder as a move, not delete+create (ADR-0199).
