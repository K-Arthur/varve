# ADR-0147: Page movement and reordering
...
ADR-0125). Both are single undo transactions.

D2 — Reordering through the Pages panel drag maps to `page.reorder` with
derived spread re-projection (ADR-0128); reordering within/across sections
...
scale content / reflow), never silent scaling (ADR-0130 D5).
...
atomically, update selection/snapping, one undo step (ADR-0126 D5).
