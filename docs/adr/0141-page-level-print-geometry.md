# ADR-0141: Page-level print geometry
...
orphaned) and export sends dialog bleed, not document bleed (ADR-0143). There
...
(sections gain `printGeometryDefaults`) → master default (ADR-0132 layer
...
(ADR-0129): LTR left page → inside = right edge; RTL mirrors. Resolved edge
