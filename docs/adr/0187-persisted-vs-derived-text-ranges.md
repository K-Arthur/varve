# ADR-0187: Persisted versus derived text ranges
...
composition key (ADR-0186 D3) and lives outside the document (in-memory
...
- Persisting ranges (fast load, diff-friendly) — rejected: ranges are
  ...
  authoritative").
- No caching (compose every frame every paint) — rejected: ADR-0188 requires
  incremental reuse.
