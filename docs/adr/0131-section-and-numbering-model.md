# ADR-0131: Section and numbering model
...
(ADR-0136); resolved values are never stored on the story.
...
`last-in-section`, `section` scopes, ADR-0132).
...
- Parity from array index (ADR-0129 D5).
