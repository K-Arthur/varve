...
| Adaptive quality | Adaptive profile fields are consumed by the hot path; unreferenced `viewportPrefetch.ts` helpers were removed 2026-09-13 | A declared prefetch/decode setting had no runtime consumer and no measured benefit | Medium performance | `adaptiveProfile.ts`, `adaptiveResidency.ts` (removal recorded in `docs/audits/chromeos-stage3-rendering-ai-2026-09-12.md` §8) | DONE for truthfulness — a future prefetch must be designed against the residency byte budget with measured evidence |
