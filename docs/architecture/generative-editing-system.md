...
the complete source-resolution candidate-mask bytes. The byte fingerprint is
computed once when inference publishes a candidate, rather than during every
render, so the safety check does not stall large-photo editors. Review
confirmation and the final Apply boundary can request an exact byte reread;
therefore changing candidates, source pixels, placement, or the selected node
invalidates review before a downstream mask or generation can consume it, and
a stale or mutated candidate cannot inherit approval from a different visible
raster.
