...
2. `apps/website/src/styles/theme.css` — the website semantic layer. It maps
   ...
   Every group has a complete light, dark and high-contrast set, plus
   `prefers-color-scheme` (no-JS), `prefers-contrast: more` (no-JS) and
   `forced-colors: active` fallbacks. `color-scheme` is set per theme so
   native controls, scrollbars and form fields match.
3. `apps/website/src/styles/global.css` — element defaults and shared
   components (`body`, headings, links, `kbd`/`pre`/`code`/tables, forms,
   `.btn-*`, `.bento-*`, `.skip-link`, `.code-block`) that only reference the
   semantic tokens.
...
- high-contrast preference wins over dark;
- the OS preference is tracked with `matchMedia` change listeners, so a
  mid-session OS switch re-themes the page;
- nothing is persisted (no manual control exists);
- with JavaScript disabled, the `prefers-color-scheme` /
  `prefers-contrast: more` fallback blocks in `theme.css` keep rendering
  readable.
...
- light-mode `--accent-primary` is a dark teal with white text (~16:1),
  ...
  dark text (~8:1); high-contrast uses a bright accent with black text.
...
- computed-style contrast audit across 26 routes x 3 themes;
- theme resolution, no-FOUC ordering, no mixed-theme rendering;
...
- 12 screenshot baselines (`visual.spec.ts`), regenerate deliberately with
