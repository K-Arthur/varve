...
| PSD | `8BPS` + version 1 | Partial document | Layer tree, bounds, visibility, opacity, groups, and representable masks remain editable; supported layer pixels are materialized as embedded PNG fills; header, dimension, encoded-size, and pixel budgets are checked before parsing; effects, adjustment layers, smart objects, and exact text editing are disclosed as losses | Yes | Parsed source metadata where available; generated layer PNGs are normalized RGBA8 | None | No PSD encoder | Parser / parser |
| PSB | `8BPS` + version 2 | Partial document | Same bounded layer-tree and embedded-PNG layer conversion as PSD; header, dimension, encoded-size, and pixel budgets are checked before parsing; large-document features outside the parser are disclosed | Yes | Parsed source metadata where available; generated layer PNGs are normalized RGBA8 | None | No PSB encoder | Parser / parser |
