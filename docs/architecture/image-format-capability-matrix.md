...
| SVG/SVGZ | XML or gzip/container path | Vector preserving | Supported paths/text/paints remain editable; ordered sibling roots and nested groups retain their source order; transforms are rebased once into the imported frames; unsafe/external resources rejected | Yes | Scene color policy; SVG metadata where safe | Animation is not exported as a motion timeline | SVG export when representable; embedding and tracing are separate semantics | Parser / parser |
...
| AI | `.ai` + `%PDF-` or `%!PS-` | Partial document | PDF-compatible and legacy PostScript wrappers use the AI adapter; complex Illustrator semantics are reported; header-only or over-budget wrappers fail without placeholder nodes | Yes | Bounding box where parsed | None | No AI encoder | Parser / parser |
| EPS | `%!PS-` / `%%BoundingBox` | Partial document | Basic paths and text from the maintained safe subset; PostScript execution, effects, patterns, gradients, and embedded fonts are not imported | Decoder-dependent | Bounding box and source declarations where parsed | None | No EPS encoder | Parser / parser |
| PSD | `8BPS` + version 1 | Partial document | Layer tree, bounds, visibility, opacity, groups, and representable masks remain editable; supported layer pixels are materialized as embedded PNG fills; header, dimension, encoded-size, and pixel budgets are checked before parsing; invalid or sentinel mask rectangles are omitted with a warning; effects, adjustment layers, smart objects, and exact text editing are disclosed as losses | Yes | Parsed source metadata where available; generated layer PNGs are normalized RGBA8 | None | No PSD encoder | Parser / parser |
| PSB | `8BPS` + version 2 | Partial document | Same bounded layer-tree and embedded-PNG layer conversion as PSD; header, dimension, encoded-size, and pixel budgets are checked before parsing; invalid or sentinel mask rectangles are omitted with a warning; large-document features outside the parser are disclosed | Yes | Parsed source metadata where available; generated layer PNGs are normalized RGBA8 | None | No PSB encoder | Parser / parser |
