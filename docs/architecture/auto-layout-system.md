...
- **Manual resize of a hug-sized frame doesn't convert it to Fixed.** Dragging a hug frame's edge bakes a new box via `TransformEngine.bakeNode`, but nothing flips its sizing mode away from `hug`, so the very next reflow (triggered by that same resize commit) re-derives the box from content again — observed directly during visual verification producing a visibly wrong intermediate size, not just "no effect." Spec explicitly calls for hug→fixed conversion on manual resize; it isn't implemented. The reverse gesture (double-click edge → back to hug) *is* implemented and unit-tested, but has nothing to revert from until this lands.
...
- **`LayoutSection.tsx`'s Width and Height sizing selects both call the same legacy `setSelectedLayoutSizing` handler** instead of the axis-specific `setSelectedLayoutSizingWidth`/`setSelectedLayoutSizingHeight` setters — found during this pass's visual verification (changing "Height sizing mode" visibly changed width instead). Left unfixed: it's on a frame's own sizing controls, not the child-layout work this pass covers, and the correct setters already exist and are exercised by `LayoutChildSection.tsx` and the new double-click-to-hug gesture above.
