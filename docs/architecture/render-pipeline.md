...
- **Circle coverage was tested in framebuffer space (fixed 2026-09-13).** The circle fragment shader compared `distance(@builtin(position).xy, center)` against `r × zoom`. That is only correct while the composed transform is conformal: a non-uniform item scale or a skew maps the object-local circle to an ellipse, and the screen-space test clipped it back to a circle of radius `r`. A circle under `transform = [2, 0, 0, 1, …]` rendered at ~55% of its Canvas2D width. `CIRCLE_VERTEX_WGSL` now forwards `localPos` as a varying and the fragment shader tests coverage in that same local space, which is exact for every affine. Verified with real AMD RDNA-2 hardware WebGPU in Chromium (`tests/e2e/webgpu/circle-transform-parity.spec.ts`) against the Canvas2D replay: uniform, `scaleX=2`, and skewed cases match coverage to within 1.5%; a singular item affine now fails closed to Canvas2D (`isGpuBatchSupported`) because a degenerate GPU triangle can still paint while Canvas2D paints nothing.
