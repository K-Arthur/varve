...
| PSD / PSB | Partial | Version 1/2 `8BPS` files import a bounded layer tree (groups, bounds, visibility, opacity, and representable masks); header, dimension, encoded-size, and pixel budgets are checked before parsing; layer pixels, effects, adjustment layers, and smart objects are reported as unsupported |
