...
| Native prototypes | `upscale_image` and `trace_image` Tauri commands | Internal `strata-upscale` and `strata-trace` crates | Source and crate tests; excluded from default dispatch |
...
- [Adobe Illustrator Image Trace](https://helpx.adobe.com/illustrator/desktop/manage-objects/traces-mockups-symbols/image-trace-panel-options.html) exposes threshold, path fidelity, corners, and noise-area concepts. Strata ships threshold, minimum area, and color/grayscale palette modes with compound-path holes.
- [Adobe Photoshop resampling options](https://helpx.adobe.com/photoshop/desktop/crop-resize-transform/resize-adjust-resolution/resampling-options.html) distinguishes nearest-neighbor hard edges from smoother photographic interpolation. Strata names the methods explicitly and does not call conventional resampling AI.
- [Adobe InDesign effective resolution guidance](https://helpx.adobe.com/indesign/desktop/troubleshoot/file-and-output-issues/pixelated-graphics.html) separates display proxies from final output and uses effective PPI at placed size. Strata does not yet expose physical-size controls because placed-image PPI is not modeled.
...
- [Canva Image Upscaler](https://www.canva.com/features/image-upscaler/) markets a model-backed one-click workflow. Strata exposes AI detail only for its live, model-backed Real-ESRGAN route.
- [Inkscape tracing guidance](https://inkscape.org/en/doc/tutorials/tracing/tutorial-tracing.html) describes Potrace-derived monochrome tracing and simplification. Strata filters small components, simplifies contours, and caps retained paths without adopting Potrace's GPL implementation.
- [ONNX Runtime Web](https://onnxruntime.ai/docs/tutorials/web/) supports WASM execution in workers. Strata uses that path for its bundled Real-ESRGAN and U2-Net Light models.
...
cargo test -p strata-upscale nearest_golden
cargo check -p strata-upscale --features ai
...
| Nearest TS/Rust parity | `upscaleGoldenParity.test.ts` + strata-upscale | shared 2x2 to 4x4 golden | Directly executed |
