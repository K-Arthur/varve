...
The offscreen surface's halo is the sum of each visible local stage's support,
in authored order. A later blur or displacement may sample pixels generated by
an earlier stage, so taking only the largest individual radius is insufficient.
Bound calculation treats non-finite malformed numeric values as zero for
allocation safety; it does not rewrite authored parameters or persisted data.
The rule is covered by `packages/engine/src/contentEffectBounds.test.ts` and is
intentionally separate from `totalEffectExpansion()` because these effects run
in the content surface's local coordinate space.
